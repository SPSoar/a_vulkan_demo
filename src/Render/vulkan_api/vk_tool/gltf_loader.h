...
{
  public:
	GLTFLoader(SPDevice &device);

	virtual ~GLTFLoader() = default;

	std::unique_ptr<SPSScene> read_scene_from_file(const std::string &file_name, int scene_index = -1);
	 ...
	 */
	std::unique_ptr<SPSSubMesh> read_model_from_file(const std::string &file_name, uint32_t index);

  protected:
	virtual std::unique_ptr<SPSNode> parse_node(const tinygltf::Node &gltf_node, size_t index) const;

	virtual std::unique_ptr<SPPCamera> parse_camera(const tinygltf::Camera &gltf_camera) const;

	virtual std::unique_ptr<SPSMesh> parse_mesh(const tinygltf::Mesh &gltf_mesh) const;

	virtual std::unique_ptr<SPSPBRMaterial> parse_material(const tinygltf::Material &gltf_material) const;

	virtual std::unique_ptr<SPSImage> parse_image(tinygltf::Image &gltf_image) const;

	virtual std::unique_ptr<SPSSampler> parse_sampler(const tinygltf::Sampler &gltf_sampler) const;

	virtual std::unique_ptr<SPSTexture> parse_texture(const tinygltf::Texture &gltf_texture) const;

	virtual std::unique_ptr<SPSPBRMaterial> create_default_material();

	virtual std::unique_ptr<SPSSampler> create_default_sampler();

	virtual std::unique_ptr<SPPCamera> create_default_camera();

	/**
	 * @brief Parses and returns a list of scene graph lights from the KHR_lights_punctual extension
	 */
	std::vector<std::unique_ptr<SPSLight>> parse_khr_lights_punctual();
  ...
  private:
	SPSScene load_scene(int scene_index = -1);

	std::unique_ptr<SPSSubMesh> load_model(uint32_t index);
};
}        // namespace vkb
